                            ...
                            <div class="tab-content mt-3">
                                ...
                                <div class="tab-pane fade @if ($loop->first) show active @endif" id="{{ $key }}" role="tabpanel" aria-labelledby="{{ $key }}-tab">
                                    <div class="form-group">
                                        ...
                                        <input type="text" id="{{ $key }}_title" name="{{ $key }}[title]" class="form-control" value="{{ $new->translate($key)->title ?? '' }}">
                                    </div>
                                    <div class="form-group">
                                        <label for="{{ $key }}_desc">المحتوى - {{ $lang }}</label>
                                        <textarea id="{{ $key }}_desc" name="{{ $key }}[desc]" class="form-control" rows="3">{{ $new->translate($key)->desc ?? '' }}</textarea>
                                    </div>
                                </div>
                                @endforeach
                            </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select id="status" name="status" class="form-control select2">
                                    <option value="active" {{ $new->status == 'active' ? 'selected' : '' }}>مفعل</option>
                                    <option value="disabled" {{ $new->status == 'disabled' ? 'selected' : '' }}>غير مفعل</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="col-md-6 mt-3">
                            <div class="form-group">
                                ...
                                <input type="number" name="order" class="form-control" value="{{ $new->order }}">
                            </div>
                        </div>
                    </div>
...
<tr>
    <td>
        <a href="{{ $image->url }}" target="_blank">
            <img src="{{ $image->url }}"
                 class="rounded" style="width:160px; max-height:100px">
        </a>
    </td>
    <td>
