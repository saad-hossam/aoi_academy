...
@section('page-header')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                <h4 class="content-title mb-0 my-auto">الوحدات</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ قائمة
                    الوحدات</span>
            </div>
        </div>
        ...
        @can('unit-create')
            ...
            <div class="d-flex my-xl-auto right-content">
                <a class="btn btn-primary btn-block" href="{{ route('units.create') }}">اضافه وحدة</a>
            </div>
        @endcan
                        ...
                        <table class="table table-hover  text-md-nowrap" id="example1" data-page-length='50'
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th class="wd-15p border-bottom-0">حالة الوحدة </th>
                                    <th class="wd-15p border-bottom-0"> الجهة التابع لها </th>
                                ...
                                @if ($units->count() > 0)
                                    @foreach ($units as $unit)
                                        <tr>
                                            <td>{{ $unit->id }}</td>
                                            <td>{{ $unit->title }}</td>
                                            {{-- <td><img src="{{ asset('images/units/' . $unit->logo) }}" alt="unit Logo"></td>
                                            --}}
                                            ...
                                            <td>
                                                @if ($unit->geha == 'تابع للهيئة')
                                                    <span class="label text-success  d-flex  " style="margin-right: 50px;">
                                                        <div class="dot-label bg-success "></div>تابع للهيئة
                                                    </span>
                                                @else
                                                    <span class="label text-danger  d-flex " style="margin-right: 50px;">
                                                        <div class="dot-label bg-danger "></div>غير تابع للهيئة
                                                    </span>
                                                @endif
                                            </td>
                                            ...
                                            <td>
                                                ...
                                                @can('unit-edit')
                                                    ...
                                                    <a href="{{ route('units.edit', $unit->id) }}" class="btn btn-sm btn-info"
                                                ...
                                                @can('unit-delete')
                                                    ...
                                                    <a class="modal-effect btn btn-sm btn-danger" data-effect="effect-scale"
                                                        data-user_id="{{ $unit->id }}" data-username="{{ $unit->title }}"
        ...
        <div class="modal" id="modaldemo8">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content modal-content-demo">
                    ...
                    <form action="{{ route('units.destroy', 0) }}" method="post">
                        {{ method_field('delete') }}
                        {{ csrf_field() }}
                        <div class="modal-body">
                            <p>هل انت متاكد من عملية الحذف ؟</p><br>
                            <input type="hidden" name="unit_id" id="user_id" value="">
